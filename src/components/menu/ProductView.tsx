...
export const ProductView: React.FC<ProductViewProps> = ({ product, relatedItems }) => {
      ...
      <div className="container mx-auto px-6 max-w-6xl">
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-start mb-24">
          <div className="relative aspect-square rounded-[40px] overflow-hidden border border-white/5 shadow-3xl lg:sticky lg:top-32">
            <Image
              src={product.image_256 ? `data:image/png;base64,${product.image_256}` : 'https://picsum.photos/seed/food/800'}
              alt={product.name}
              fill
          ...
          <div className="space-y-10 pt-4">
            <div>
              <div className="flex items-center gap-4 mb-6">
                <Badge variant="outline" className="text-accent-gold border-accent-gold/20 px-4 py-1">
                  {product.category}
                </Badge>
                {product.isFeatured && (
                  <Badge className="bg-accent-gold text-primary px-4 py-1">Chef&apos;s Recommendation</Badge>
                )}
              </div>
            ...
            <div className="space-y-6">
              ...
              <div className="flex items-baseline gap-4">
                ...
                <span className="text-white/20 text-sm uppercase tracking-widest">Base Price</span>
              </div>
            </div>
        ...
        {relatedItems.length > 0 && (
          <div className="space-y-12">
            <div className="flex items-end justify-between border-b border-white/5 pb-8">
              <div>
                <h3 className="text-accent-gold uppercase tracking-[0.3em] text-[10px] mb-2">Wait, there is more</h3>
                <h2 className="text-3xl md:text-4xl font-display font-bold text-white">Suggested for You</h2>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
              {relatedItems.map((item) => (
                <Link
                  key={item.id}
                  href={`/menu/${item.name.toLowerCase().replace(/ /g, '-').replace(/[^\w-]+/g, '')}`}
                  className="group bg-neutral-900/40 rounded-[32px] border border-white/5 overflow-hidden hover:border-accent-gold/30 transition-all duration-500"
                >
                  <div className="relative h-64 overflow-hidden bg-neutral-800">
                    <Image
                      src={item.image_256 ? `data:image/png;base64,${item.image_256}` : 'https://picsum.photos/seed/food/400'}
                      alt={item.name}
                      fill
                  ...
                  <div className="p-8">
                    <div className="flex justify-between items-start mb-2">
                      <h4 className="text-xl font-display font-bold text-white group-hover:text-accent-gold transition-colors">{item.name}</h4>
                      <span className="text-accent-gold font-bold">{formatPrice(item.list_price)}</span>
                    </div>
                    {item.details?.description_sale && <p className="text-white/40 text-sm line-clamp-2 italic">&quot;{item.details.description_sale}&quot;</p>}
                  </div>
                </Link>
              ))}
            </div>
