...
export const ProductCard: React.FC<ProductCardProps> = ({
    ...
    return (
        <div
            className={cn(
                "group bg-neutral-900/40 rounded-[32px] border border-white/5 overflow-hidden hover:border-accent-gold/30 transition-shadow duration-500 flex flex-col",
        ...
        >
            ...
            <div className="p-6 md:p-8 flex flex-col flex-grow">
                <div className="flex justify-between items-start mb-2">
                    <p className="text-xl font-display font-bold text-white leading-tight">{product.name}</p>
                    <span className="text-accent-gold font-bold ml-2">{formatPrice(product.list_price)}</span>
                </div>
                ...
                <div className="mt-auto">
                    <div className="flex gap-2">
                        <Button
                            variant="outline"
                            size="sm"
                            className="flex-grow rounded-2xl border-white/10 group-hover:bg-accent-gold group-hover:text-primary transition-all"
                            disabled={!isPosOpen}
                            onClick={(e) => {
                                e.stopPropagation();
                        ...
                        <Link href={`/menu/${generateSlug(product.name)}`} className="w-1/3" onClick={(e) => e.stopPropagation()}>
                            <Button
                                variant="ghost"
                                size="sm"
