    ...
    <footer role="contentinfo" className="bg-neutral-950 border-t border-white/5 pt-24 pb-12">
      <div className="container mx-auto px-6">
        <div className="grid md:grid-cols-4 gap-12 mb-20">
          <div className="col-span-2">
            ...
            <p className="text-white/70 max-w-sm mb-8 leading-relaxed italic">
              {t('brandQuote', { year: RAM_GROUP.established })}
            </p>
          ...
          <div>
            <p className="text-accent-gold uppercase tracking-widest text-xs font-bold mb-8">{t('branches')}</p>
            <ul className="space-y-4 text-white/70 text-sm">
              {RAM_GROUP.stores.map(store => (
                <li key={store.id}>
                  <Link href="/access" className="hover:text-white transition-colors">{translate(store.name)}</Link>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div>
            <p className="text-accent-gold uppercase tracking-widest text-xs font-bold mb-8">{t('links')}</p>
            <ul className="space-y-4 text-white/70 text-sm">
              <li><Link href="/booking?tab=party" className="hover:text-white transition-colors">{t('party')}</Link></li>
              <li><Link href="/menu" className="hover:text-white transition-colors">{t('takeout')}</Link></li>
              <li><Link href="/team" className="hover:text-white transition-colors">{t('career')}</Link></li>
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-6 text-[10px] uppercase tracking-[0.4em] text-white/70">
          <p>© 1999 - 2024 {RAM_GROUP.name}. {t('rights')}</p>
          <p>{t('authentic')}</p>
        </div>
      </div>
    </footer>
