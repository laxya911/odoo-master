...
export default function CheckoutPage() {
    ...
    if (items.length === 0) {
        return (
            <div className="container mx-auto px-4 py-32 text-center">
                ...
                <h1 className="text-3xl font-bold mb-4">{t('bagEmpty')}</h1>
                <Button onClick={() => router.push('/menu')} variant="default" className="rounded-full px-8 h-12">{t('returnMenu')}</Button>
            </div>
        );
    }
    ...
    if (isProcessing) {
        return (
            <div className="container mx-auto px-4 py-40 text-center animate-in fade-in zoom-in duration-500">
                ...
                <h1 className="text-4xl font-bold mb-4 font-headline">{t('preparing')}</h1>
    ...
    return (
        <div className="container mx-auto px-4 py-24 max-w-6xl">
            <h1 className="text-4xl lg:text-6xl font-bold mb-12 font-headline">{t('finish')}</h1>
                ...
                <div className="flex-1 space-y-12">
                    ...
                    <section className="space-y-6">
                        <div className="flex items-center gap-4">
                            ...
                            <h2 className="text-2xl font-bold font-headline uppercase tracking-wider">{t('personal')}</h2>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-8 lg:p-12 bg-white rounded-[3rem] shadow-sm border border-accent-gold/5">
                            <div className="space-y-2 md:col-span-2">
                                <Label className="text-xs uppercase tracking-widest font-bold opacity-50">{t('street')}</Label>
                                <Input
                            ...
                            <div className="space-y-2">
                                <Label className="text-xs uppercase tracking-widest font-bold opacity-50">{t('city')}</Label>
                                <Input
                            ...
                            <div className="space-y-2">
                                <Label className="text-xs uppercase tracking-widest font-bold opacity-50">{t('zip')}</Label>
                                <Input
                            ...
                            <div className="space-y-2">
                                <Label className="text-xs uppercase tracking-widest font-bold opacity-50">{t('phone')}</Label>
                                <Input
                    ...
                    <section className="space-y-6">
                        <div className="flex items-center gap-4">
                            ...
                            <h2 className="text-2xl font-bold font-headline uppercase tracking-wider">{t('paymentMethod')}</h2>
                        </div>
                        <PaymentGateway
                            ...
                            <div className="p-8 lg:p-12 bg-white rounded-[3rem] shadow-sm border border-accent-gold/5 space-y-8 mt-8">
                                <div className="flex items-center gap-6">
                                    ...
                                    <div className="space-y-1">
                                        <h3 className="text-2xl font-bold">{t('cardMethod')}</h3>
                        ...
                        {paymentMethod === 'cash' && (
                            <div className="p-8 lg:p-12 bg-neutral-50 rounded-[3rem] text-center space-y-6 border border-dashed border-neutral-200 mt-8">
                                ...
                                <div className="space-y-2">
                                    <h3 className="text-2xl font-bold">{t('cashMethod')}</h3>
                                    <p className="text-muted-foreground leading-relaxed">
                                        {t('cashMethodDesc')}
                                    </p>
                                </div>
                    ...
                    <div className="sticky top-32 space-y-6">
                        <Card className="rounded-[3rem] shadow-2xl border-none overflow-hidden bg-neutral-900 text-white">
                            <CardHeader className="bg-white/5 p-8 border-b border-white/10">
                                <CardTitle className="text-xl font-bold font-headline tracking-wider uppercase">{t('orderSummary')}</CardTitle>
                            </CardHeader>
                            <CardContent className="p-8 space-y-8">
                                    ...
                                    {items.map(item => (
                                        <div key={item.id} className="group">
                                            <div className="flex justify-between items-start gap-4 mb-1">
                                                <p className="font-bold flex-1 text-sm">{item.quantity}x {translate(item.product.name)}</p>
                                ...
                                <div className="pt-6 border-t border-white/10 space-y-4">
                                    ...
                                    <p className="text-[10px] text-white/30 text-center">{t('taxIncluded')}</p>
                                </div>
