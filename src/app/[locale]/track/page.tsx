...
const STATUS_STEPS: { status: OrderStatus; key: string; icon: React.ElementType }[] = [
    ...
    { status: 'preparing', key: 'kitchen', icon: ChefHat },
    { status: 'ready', key: 'packing', icon: Utensils },
    { status: 'delivering', key: 'out', icon: Bike },
    { status: 'delivered', key: 'arrived', icon: CheckCircle },
];
    ...
    if (!lastOrder) {
        return (
            <div className="container mx-auto px-4 py-20 text-center space-y-6">
                ...
                <h1 className="text-3xl font-bold font-headline">{t('noActive')}</h1>
                <p className="text-muted-foreground max-w-sm mx-auto">{t('noActiveDesc')}</p>
                <Link href="/menu">
                    <Button className="rounded-full px-8 h-12 bg-accent-gold hover:bg-accent-gold/90 text-primary-foreground">{t('browse')}</Button>
                </Link>
            </div>
        );
    }
        ...
        <div className="container mx-auto px-4 py-12 max-w-6xl">
            <div className="text-center mb-16 space-y-2">
                <h1 className="text-5xl font-bold font-headline">
                    {t('title').includes(' ') ? (
                        <>
                            {t('title').split(' ')[0]} <span className="text-accent-gold">{t('title').split(' ')[1]}</span>
                        </>
                    ) : (
                        t('title')
                    )}
                </h1>
                <p className="text-muted-foreground font-bold tracking-widest text-sm">{t('orderId')}: #{lastOrder.id}</p>
            </div>
                    ...
                    <Card className="rounded-[3rem] border-none shadow-2xl overflow-hidden">
                        <CardHeader className="bg-accent-gold text-white p-10">
                            <div className="flex justify-between items-center">
                                <div>
                                    <CardTitle className="text-3xl font-headline">{t('estDelivery')}</CardTitle>
                                    <CardDescription className="text-white/80 text-lg">{t('estTime')}</CardDescription>
                                </div>
                            ...
                            <div className="pt-10 border-t border-dashed space-y-6">
                                <div className="flex items-start gap-5">
                                    ...
                                    <div className="space-y-1">
                                        <p className="font-bold text-lg font-headline">{t('destination')}</p>
                        ...
                        <CardContent className="p-10">
                            <div className="flex items-center gap-8">
                                ...
                                <div className="flex-1 space-y-2">
                                    <p className="text-xs font-bold text-accent-gold uppercase tracking-widest">{t('tandoorMaster')}</p>
                                    <h3 className="text-2xl font-bold font-headline">{t('masterChef')}</h3>
                                    <div className="flex items-center gap-2 text-green-600">
                                        <CheckCircle className="w-5 h-5 fill-current" />
                                        <span className="text-sm font-bold text-foreground">{t('verifiedPartner')}</span>
                                    </div>
                                </div>
                                <Button variant="outline" className="rounded-full px-8 h-12 border-accent-gold text-accent-gold hover:bg-accent-gold hover:text-white font-bold transition-all">{t('supportChat')}</Button>
                            </div>
                        </CardContent>
                    </Card>
                </div>
                ...
                <div className="space-y-8">
                    <Card className="rounded-[2.5rem] border-none shadow-2xl sticky top-24 bg-[#FAF4F0]">
                        <CardHeader className="p-8 pb-4">
                            <CardTitle className="font-headline text-2xl">{t('orderSummary')}</CardTitle>
                        </CardHeader>
                        <CardContent className="p-8 space-y-6">
                                ...
                                {lastOrder.items.map((item, idx) => (
                                    <div key={idx} className="flex justify-between items-start text-sm gap-4">
                                        <div className="flex-1">
                                            <p className="font-bold">{item.quantity}x {translate(item.product.name)}</p>
                                            {item.selectedAttributes && Object.entries(item.selectedAttributes).map(([attrId, valId]) => {
                                                ...
                                                return valueIds.map((vid: number) => {
                                                    ...
                                                    return val ? <p key={`${attrId}-${vid}`} className="text-[10px] text-muted-foreground uppercase">{translate(val.name)}</p> : null;
                                                });
                                            })}
                                            ...
                                            {!item.selectedAttributes && item.meta?.attribute_value_ids?.map((vid: number) => {
                                                ...
                                                item.product.attributes?.forEach((attr: ProductAttribute) => {
                                                    const val = attr.values.find((v: { id: number; name: string }) => String(v.id) === String(vid));
                                                    if (val) foundValName = translate(val.name);
                                                });
                            ...
                            <div className="pt-6">
                                <div className="bg-white p-6 rounded-3xl flex items-center gap-4 shadow-sm">
                                    ...
                                    <div>
                                        <p className="text-xs font-bold uppercase tracking-widest">{t('paymentSuccess')}</p>
