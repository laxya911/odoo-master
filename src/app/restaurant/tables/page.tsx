...
import { TablesClient } from './client';
import { TablesLoading } from './loading';
import type { Paginated, OdooRecord, OdooError } from '@/lib/types';
import { Alert, AlertDescription, AlertTitle } from '@/components/ui/alert';
import { AlertTriangle } from 'lucide-react';

async function getPaginatedData(resource: string, searchParams: URLSearchParams): Promise<Paginated<OdooRecord> | { error: OdooError }> {
  const url = new URL(`${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'}/api/odoo/restaurant/${resource}`);
